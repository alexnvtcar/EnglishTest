...
async function installPWA() {
    ...
    // Для Android и Desktop - принудительная установка
    try {
        // Сначала пробуем использовать deferredPrompt
        if (deferredPrompt) {
            console.log('📱 Используем deferredPrompt для установки');
            ...
            if (outcome === 'accepted') {
                ...
                showNotification('Установка отменена', 'info');
            }
        ...
        // Если deferredPrompt недоступен, пробуем принудительную установку
        console.log('📱 Принудительная установка PWA...');
        
        // Показываем уведомление о попытке установки
        showNotification('Попытка установки...', 'info');
        
        // Пробуем несколько способов установки
        await tryMultipleInstallMethods();
        
    } catch (error) {
        console.error('Ошибка установки PWA:', error);
        showNotification('Ошибка установки. Показываем инструкции...', 'warning');
...
// Множественные способы установки PWA
async function tryMultipleInstallMethods() {
    console.log('📱 Пробуем разные способы установки...');
    
    // Проверяем, что мы на HTTP/HTTPS, а не file://
    if (window.location.protocol === 'file:') {
        showNotification('Для установки PWA откройте приложение через HTTP сервер!', 'warning');
        showManualInstallInstructions();
        return;
    }
    
    // Способ 1: Прямая попытка установки через браузер
    console.log('📱 Способ 1: прямая установка');
    try {
        // Показываем диалог установки
        const installConfirmed = confirm('Установить приложение "English Learning"?');
        
        if (installConfirmed) {
            // Пробуем использовать deferredPrompt если он есть
            if (deferredPrompt) {
                console.log('📱 Используем сохраненный deferredPrompt');
                deferredPrompt.prompt();
                const { outcome } = await deferredPrompt.userChoice;
            ...
            // Если deferredPrompt нет, показываем инструкции
            showNotification('Автоматическая установка недоступна. Показываем инструкции...', 'info');
            setTimeout(() => {
                showManualInstallInstructions();
            }, 1000);
    ...
    // Способ 2: Попытка через изменение URL
    console.log('📱 Способ 2: изменение URL');
    try {
        const currentUrl = window.location.href;
        const newUrl = currentUrl + (currentUrl.includes('?') ? '&' : '?') + 'install=true';
        
        // Временно изменяем URL для принудительной установки
    ...
    // Способ 3: Попытка через service worker
    console.log('📱 Способ 3: service worker');
    try {
        if ('serviceWorker' in navigator) {
        ...
        console.log('Способ 3 не сработал:', e);
    }
    
    // Способ 4: Попытка через манифест
    console.log('📱 Способ 4: обновление манифеста');
    try {
        ...
        console.log('Способ 4 не сработал:', e);
    }
    
    // Ждем результат установки
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    // Проверяем, установилось ли приложение
    if (window.matchMedia('(display-mode: standalone)').matches) {
        isPWAInstalled = true;
        updatePWAInstallButton();
        showNotification('Приложение установлено!', 'success');
    } else {
        // Если не установилось, показываем инструкции
        showNotification('Автоматическая установка не удалась. Показываем инструкции...', 'warning');
        setTimeout(() => {
            showManualInstallInstructions();
        }, 1000);
    }
        ...
        <div class="modal-content" style="max-width: 500px;">
            ...
            <div style="padding: 20px;">
                <div style="margin-bottom: 20px;">
                    <p><strong>Для установки приложения на iPhone или iPad:</strong></p>
                </div>
                <div style="margin-bottom: 15px;">
                    <div style="display: flex; align-items: center; margin-bottom: 10px;">
                        ...
                        <span>Нажмите кнопку <strong>"Поделиться"</strong> внизу экрана</span>
                    </div>
                    <div style="display: flex; align-items: center; margin-bottom: 10px;">
                ...
                <div style="background: #f0f9ff; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                    <p style="margin: 0; font-size: 14px; color: #1e40af;">
                        💡 <strong>Совет:</strong> После установки приложение появится на главном экране как обычное приложение!
                    </p>
                </div>
            </div>
...
function showManualInstallInstructions() {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 600px;">
            ...
            <div style="padding: 20px;">
                <div style="margin-bottom: 20px;">
                    <p><strong>Для установки приложения на ваш компьютер:</strong></p>
                </div>
                ...
                <div style="margin-bottom: 20px; padding: 15px; background: #f8fafc; border-radius: 8px; border-left: 4px solid #1e40af;">
                    ...
                    <div style="display: flex; align-items: center; margin-bottom: 8px;">
                        ...
                        <span>Найдите иконку <strong>"Установить"</strong> в адресной строке (справа)</span>
                    </div>
                    <div style="display: flex; align-items: center; margin-bottom: 8px;">
                        ...
                        <span>Или меню браузера → <strong>"Установить приложение"</strong></span>
                    </div>
                </div>
                ...
                <div style="margin-bottom: 20px; padding: 15px; background: #f8fafc; border-radius: 8px; border-left: 4px solid #ff6b35;">
                    ...
                    <div style="display: flex; align-items: center; margin-bottom: 8px;">
                        ...
                        <span>Меню браузера → <strong>"Установить"</strong></span>
                    </div>
                </div>
                ...
                <div style="margin-bottom: 20px; padding: 15px; background: #f8fafc; border-radius: 8px; border-left: 4px solid #007aff;">
                    ...
                    <div style="display: flex; align-items: center; margin-bottom: 8px;">
                        ...
                        <span>Меню "Файл" → <strong>"Добавить на экран Домой"</strong></span>
                    </div>
                </div>
                
                <div style="background: #f0f9ff; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                    <p style="margin: 0; font-size: 14px; color: #1e40af;">
                        💡 <strong>Важно:</strong> Убедитесь, что приложение открыто через HTTP сервер (http://localhost:8000), а не как файл!
                    </p>
                </div>
                
                <div style="background: #fef3c7; padding: 15px; border-radius: 8px;">
                    <p style="margin: 0; font-size: 14px; color: #92400e;">
                        ⚠️ <strong>Если иконка установки не появляется:</strong> Обновите страницу (F5) и подождите несколько секунд.
                    </p>
                </div>
            </div>
            <div style="display: flex; gap: 12px; justify-content: flex-end; margin-top: 20px; padding: 0 20px 20px;">
                <button class="btn btn-secondary" onclick="this.closest('.modal').remove()">Закрыть</button>
                <button class="btn btn-warning" onclick="forceInstallPWA()">⚡ Установить принудительно</button>
...
// Принудительная установка PWA
async function forceInstallPWA() {
    console.log('⚡ Принудительная установка PWA...');
    
    // Проверяем, что мы на HTTP/HTTPS
    if (window.location.protocol === 'file:') {
        showNotification('Для установки PWA откройте приложение через HTTP сервер!', 'error');
        return;
    }
    
    showNotification('Запуск принудительной установки...', 'info');
    
    try {
        // Способ 1: Попытка через beforeinstallprompt
        if (deferredPrompt) {
            console.log('⚡ Используем deferredPrompt для принудительной установки');
            deferredPrompt.prompt();
            const { outcome } = await deferredPrompt.userChoice;
        ...
        // Способ 2: Принудительное создание события установки
        console.log('⚡ Создаем принудительное событие установки');
        const installEvent = new CustomEvent('beforeinstallprompt', {
            detail: {
                prompt: async () => {
                    return new Promise((resolve) => {
                        const confirmed = confirm('Установить приложение "English Learning" принудительно?');
        ...
        // Способ 3: Попытка через изменение манифеста
        console.log('⚡ Обновляем манифест для принудительной установки');
        const manifestLink = document.querySelector('link[rel="manifest"]');
        if (manifestLink) {
            const originalHref = manifestLink.href;
        ...
        // Способ 4: Попытка через service worker
        console.log('⚡ Отправляем сообщение service worker');
        if ('serviceWorker' in navigator) {
            try {
                const registration = await navigator.serviceWorker.ready;
                if (registration && registration.active) {
                ...
                console.log('Service worker сообщение не сработало:', e);
            }
        }
        
        // Способ 5: Попытка через изменение URL
        console.log('⚡ Изменяем URL для принудительной установки');
        const currentUrl = window.location.href;
        ...
        // Способ 6: Попытка через создание скрытого iframe
        console.log('⚡ Создаем скрытый iframe для принудительной установки');
        ...
        if (window.matchMedia('(display-mode: standalone)').matches) {
            isPWAInstalled = true;
            updatePWAInstallButton();
            showNotification('Приложение установлено принудительно!', 'success');
        } else {
            showNotification('Принудительная установка не удалась. Попробуйте ручную установку.', 'warning');
        }
        
    } catch (error) {
        console.error('Ошибка принудительной установки:', error);
        showNotification('Ошибка принудительной установки: ' + error.message, 'error');
    }
}

// Обновить состояние кнопки установки
function updatePWAInstallButton() {
    const installItem = document.getElementById('pwaInstallMenuItem');
    if (installItem) {
        if (isPWAInstalled || window.matchMedia('(display-mode: standalone)').matches) {
            installItem.innerHTML = '✅ Приложение установлено';
            installItem.style.color = '#10b981';
            installItem.onclick = null;
        } else {
            installItem.innerHTML = '📱 Установить приложение';
...
// Функция выхода из приложения (глобальная)
